...
/* max rows per GraphQL request (same as old logic) */
const CHUNK = 1000;

/* helper: parse "5m", "1h", "1d" => milliseconds */
...
export async function pairCandlesHandler(request /*, ctx */) {
  try {
    ...
    const now    = Date.now();
    const since  = now - intervalMs(range);
    const sinceIso = new Date(since).toISOString().replace("Z","");

    /* ── stream swaps in DESC order, chunked ─────────────────── */
    const qry = `
    ...
    const candles = new Map();     // key = bucket start ms

    const addCandle = (tMs, price0, vol0, vol1) => {
      const b = candles.get(tMs) || {
        t   : new Date(tMs).toISOString(),
        open: null, high: 0, low: Infinity, close: null,
        volume0:0, volume1:0
      };
      if (b.open===null) b.open = price0;
      b.high   = Math.max(b.high, price0);
      b.low    = Math.min(b.low,  price0);
      b.close  = b.close===null ? price0 : b.close; // first seen in DESC stream = close
      b.volume0 += vol0;
      b.volume1 += vol1;
      candles.set(tMs, b);
    };
    ...
    while (!done) {
      ...
      for (const { node:{ created, data } } of edges) {
        const ts = Date.parse(created);          // ms
        const bucketStart = Math.floor(ts / ivMs) * ivMs;

        const price0 = calcPrice0(data);
        if (price0===null) continue;

        const vol0 = parseFloat(data.amount0In||0)+parseFloat(data.amount0Out||0);
        const vol1 = parseFloat(data.amount1In||0)+parseFloat(data.amount1Out||0);

        addCandle(bucketStart, price0, vol0, vol1);
      }
